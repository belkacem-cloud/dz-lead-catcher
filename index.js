...
const rsleep = () => sleep(Math.floor(Math.random() * (SLEEP_MAX_MS - SLEEP_MIN_MS)) + SLEEP_MIN_MS);
...
async function collectPageLinks(page) {
  ...
  const candidates = anchors.filter(h =>
    h.startsWith('https://www.facebook.com/') &&
    !h.includes('/ads/library') && !h.includes('/ads/about') &&
    !h.includes('sharer.php') && !h.includes('/help/') && !h.includes('/policies/')
  );
  const simple = candidates.map(h => {
    try {
      const u = new URL(h);
...
async function scrapeKeyword(browser, keyword) {
  ...
  try {
    ...
    for (const b of buttons) {
      ...
      if (txt.includes('accept') || txt.includes('allow all') || txt.includes('allow essential') || txt.includes('قبول')) { await b.click(); break; }
    }
  } catch {}
...
async function main() {
  const keywords = fs.readFileSync(path.join(process.cwd(), 'keywords.txt'), 'utf8')
    .split('\n').map(s => s.trim()).filter(Boolean);

  const browser = await puppeteer.launch({ headless: HEADLESS, userDataDir: USER_DATA_DIR });
  const all = new Set();
  ...
  fs.writeFileSync('pages.json', JSON.stringify({ ts: Date.now(), pages: [...all] }, null, 2));
